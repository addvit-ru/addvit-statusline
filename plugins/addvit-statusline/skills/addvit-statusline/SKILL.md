...
description: Установить продвинутый statusLine для Claude Code. Показывает имя модели, прогресс-бар контекстного окна с процентом, use/total в токенах, кумулятивные session-токены, API rate-limits 5h/7d с цветным процентом и обратным отсчётом до сброса окна в формате "5h:24% (2h34m)" и "7d:41% (3d5h)". Плюс cwd и git-ветка. Используй когда пользователь просит "установить statusline", "поставить красивый статусбар", "хочу такой же statusline", "statusline с rate-limits", "statusline с countdown", или когда переносит этот skill в новое окружение и нужно его активировать. Работает на macOS/Linux из коробки, на Windows требует Git Bash (идёт с Git for Windows) или WSL.
---
...
```
Opus 4.7 │ ███░░░░░ 42% │ 425k/1000k │ sess:850k │ 5h:24% (2:33:05) 7d:41% (3d5h) │ D:/project │ (main)
```
...
| 5 | API rate-limits: `5h:XX% (HhMMm)` и `7d:YY% (NdHh)` — процент использования + оставшееся время до сброса окна | `rate_limits.five_hour.*`, `rate_limits.seven_day.*` (только Pro/Max после первого ответа API) |
